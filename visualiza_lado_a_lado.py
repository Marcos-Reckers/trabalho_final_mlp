...
def render_state(entry, color):
    ...
    for ar in reversed(stack):
        ...
        panel = Panel(locals_table, title=f"[bold magenta]{ar['name']}[/] (type={ar['scope_type']}, lex={ar['lex_parent']}, parent={ar['parent']})", border_style="magenta", padding=(0,1))
        stack_panels.append(panel)
    if stack_panels:
        stack_render = Group(*stack_panels)
        stack_panel = Panel(stack_render, title="[bold]Call Stack (Top → Base)", border_style="cyan", padding=(0,1))
    else:
        stack_panel = Panel("(empty)", title="Call Stack", border_style="cyan", padding=(0,1))
    ...
    global_panel = Panel(global_table, title="🌎 [bold blue]Global Scope[/]", border_style="blue", padding=(0,1))
    # Painel principal
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Visualiza execuções static e dynamic lado a lado com cores.")
    parser.add_argument('--static-log', required=True, help='Arquivo de log JSONL do modo static')
    parser.add_argument('--dynamic-log', required=True, help='Arquivo de log JSONL do modo dynamic')
    parser.add_argument('--delay', type=float, default=1.0, help='Delay entre etapas (segundos)')
    ...
    if console.size.width < min_width:
        console.print(f"[bold red]Aumente a largura do terminal para pelo menos {min_width} colunas para ver lado a lado! (atual: {console.size.width})[/]")
        time.sleep(3)
    for i in range(n):
        panels = [
